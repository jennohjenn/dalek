<?php
/**
* Main class for Dalek, holds everything.
*
* @package DalekCore
*/
class CDalek implements ISingleton {
        ...
        protected function __construct() {
                ...
                // include the site specific config.php and create a ref to $da to be used by config.php
                $da = &$this;
    require(DALEK_SITE_PATH.'/config.php');
...
*/
  public function ThemeEngineRender() {
    ...
    $themePath         = DALEK_INSTALL_PATH . '/' . $this->config['theme']['path'];
    ...
    if(isset($this->config['theme']['parent'])) {
      $parentPath = DALEK_INSTALL_PATH . '/' . $this->config['theme']['parent'];
      $parentUrl        = $this->request->base_url . $this->config['theme']['parent'];
    }
    
    // Add stylesheet name to the $da->data array
    $this->data['stylesheet'] = $this->config['theme']['stylesheet'];
    
    // Make the theme urls available as part of $da
    ...
    $da = &$this;
    // First the default Dalek themes/functions.php
    include(DALEK_INSTALL_PATH . '/themes/functions.php');
...
*/
  ...
  /**
* Draw HTML for a menu defined in $da->config['menus'].
*
