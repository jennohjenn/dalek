...
<a href="http://validator.w3.org/checklink?uri={$da->request->current_url}">links</a>
<a href="http://qa-dev.w3.org/i18n-checker/index?async=false&amp;docAddr={$da->request->current_url}">i18n</a>
...
function login_menu() {
  $da = CDalek::Instance();
  if($da->user['isAuthenticated']) {
    $items = "<a href='" . create_url('user/profile') . "'><img class='gravatar' src='" . get_gravatar(20) . "' alt=''> " . $da->user['acronym'] . "</a> ";
    if($da->user['hasRoleAdministrator']) {
...
function get_gravatar($size=null) {
  return 'http://www.gravatar.com/avatar/' . md5(strtolower(trim(CDalek::Instance()->user['email']))) . '.jpg?r=pg&amp;d=wavatar&amp;' . ($size ? "s=$size" : null);
}
...
function create_url($urlOrController=null, $method=null, $arguments=null) {
  return CDalek::Instance()->CreateUrl($urlOrController, $method, $arguments);
}
...
function theme_url($url) {
  return create_url(CDalek::Instance()->themeUrl . "/{$url}");
}
...
function theme_parent_url($url) {
  return create_url(CDalek::Instance()->themeParentUrl . "/{$url}");
}
...
function render_views($region='default') {
  return CDalek::Instance()->views->Render($region);
}
...
function region_has_content($region='default' /*...*/) {
  return CDalek::Instance()->views->RegionHasView(func_get_args());
}
